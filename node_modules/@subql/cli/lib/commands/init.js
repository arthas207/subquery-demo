...
(function (NETWORK_FAMILIES) {
    NETWORK_FAMILIES["Avalanche"] = "Avalanche";
    NETWORK_FAMILIES["Substrate"] = "Substrate";
    NETWORK_FAMILIES["Terra"] = "Terra";
...
class Init extends core_1.Command {
    async run() {
        ...
        if ((0, semver_1.lt)(flags.specVersion, RECOMMEND_VERSION)) {
            this.log(`${chalk_1.default.yellow('WARNING')} Using specVersion ${flags.specVersion} is deprecated and in the future will be denied from being uploaded to the subquery hosted service. Consider initializing your project with specVersion ${RECOMMEND_VERSION}`);
        }
        ...
        let selectedNetwork;
        templates = (await (0, init_controller_1.fetchTemplates)()).filter(({ specVersion }) => specVersion === flags.specVersion);
    ...
    async setupProject(flags) {
        ...
        if (this.networkFamily === NETWORK_FAMILIES.Substrate && flags.specVersion !== '0.0.1') {
            ...
            this.project.genesisHash = await (0, jsonrpc_1.getGenesisHash)(this.project.endpoint);
...
Init.flags = {
    ...
    specVersion: core_1.Flags.string({
        required: false,
        options: ['0.2.0', '1.0.0'],
